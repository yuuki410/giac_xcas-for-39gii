...
#include "giacPCH.h"
#if 1 // assumes that kmalloc checks
const size_t stackptr=0xffffffff,stackmask=0x1ffff;
#else
const size_t stackptr=0x1a000,stackmask=0x1ffff;
#endif
...
#define FXCG35
...
#if 1 // def FXCG35
  const int ALLOC24=4*32;
  const int ALLOC32=3*32;
  const int ALLOC16=6*32;
  ...
  const int ALLOC32=32*32;
  const int ALLOC16=32*32;
#define ALLOC44 32*44
  static unsigned int freeslot24[ALLOC24/32]={
    0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
    0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
    0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
  ...
  static void deletefast(void* obj){
    ...
    if ( ((size_t)obj>=(size_t) &tab44[0] ) &&
      ...
      * (unsigned *) obj= 0;
      int pos= ((size_t)obj -((size_t) &tab44[0]))/sizeof(four_int);
  ...
  */
  ...
  void sprintfdouble(char * ch,const char * format,double d){
    ...
    /*
    long intPart = (long)d;
    double fracPart = (double)d - (double)intPart;
    long fracPart_i = fracPart * 100000;
    
    sprintf(ch, "%d.%d", intPart, fracPart_i);
    std::cout << "dtostr:" << ch << ", frac:" << fracPart_i << endl;*/
...
#define debugtypeptr _CPLXptr
  ...
  gen::gen(int i): val(i),subtype(0),type(_INT_) {
#ifdef COMPILE_FOR_STABILITY
      control_c();
#endif
  }
...
#endif
    if ( (((size_t) _ZINTptr) &0x1ffff) > stackptr)
...
#endif
    if ( v.size()>1 &&
	 ( (((size_t) _VECTptr) &0x1ffff) > stackptr ||
	   (((size_t) _VECTptr->begin()) &0x1ffff) > stackptr )
	 )
      ctrl_c=interrupted=true;
    type=_VECT;
  ...
  gen::gen(const symbolic & s){
    ...
    * ((ulonglong * ) this) = ulonglong(new_ref_symbolic(s)) << 16;
    ...
    type = _SYMB;
    subtype = 0;
    if ( _SYMBptr->sommet!=at_restart && _SYMBptr->sommet!=at_purge && (((size_t) _SYMBptr) &0x1ffff) > stackptr)
      ctrl_c=interrupted=true;
  }
...
#endif
	if ( (((size_t) _POLYptr) &0x1ffff) > stackptr)
	  ctrl_c=interrupted=true;
	type = _POLY;
      }
    }
  ...
  bool ref_mpz_t2gen(ref_mpz_t * mptr,gen & g){
	...
	return false;
      ...
      * ((ulonglong * ) &g) = ulonglong(mptr) << 16;
      ...
      g.__ZINTptr = mptr;
#endif
      if ( (((size_t) g._ZINTptr) &0x1ffff) > stackptr)
	ctrl_c=interrupted=true;
      g.type =_ZINT;
      g.subtype=0;
      return true;
  ...
  gen::gen(const sparse_poly1 & p){
	...
	* ((ulonglong * ) this) = ulonglong(new ref_sparse_poly1(p)) << 16;
	...
	subtype=0;
	type=_SPOL1;
	if ( (((size_t) _SPOL1ptr) &0x1ffff) > stackptr)
	  ctrl_c=interrupted=true;
      }
    }
  }
